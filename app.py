...
# --- 2. THE DATA ENGINE ---
def load_data():
    # Use your 120-entry dataset here
    # Expected columns: 'IPO Name', 'Listing Gain %', 'Subscription (x)', 'GMP', 'Sector'
    try:
        df = pd.read_csv("ipo_data.csv")
        return df
    except:
        st.error("Please ensure 'ipo_data.csv' is in the folder.")
        return pd.DataFrame()
...
if not df.empty:
    ...
    fig_hist = px.histogram(final_prices, nbins=50, labels={'value': 'Final Value'})
